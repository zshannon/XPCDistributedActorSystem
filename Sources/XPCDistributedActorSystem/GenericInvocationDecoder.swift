...
public struct GenericInvocationDecoder: DistributedTargetInvocationDecoder
{
    ...
    {
        self.argumentsIterator = request.arguments.makeIterator()
    ...
    {
        self.request.genericSubstitutions.compactMap({ _typeByName($0) })
    }

    public mutating func decodeErrorType() throws -> (Any.Type)?
    {
        guard let errorTypeMangled = self.request.errorType else { return nil }
        return _typeByName(errorTypeMangled)
    }

    public mutating func decodeReturnType() throws -> (Any.Type)?
    {
        guard let returnTypeMangled = self.request.returnType else { return nil }
