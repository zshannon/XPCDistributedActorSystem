...
enum SigningInformationExtractor {
    ...
    static func getCurrentTeamID() throws -> String {
        ...
        try withStatusCheck(SecCodeCopySigningInformation(
            codeOnDisk,
            signingInformationFlags,
            &extractedSigningInformation,
        ))
        guard let extractedSigningInformation = extractedSigningInformation as? [String: Any],
              let teamIdentifier = extractedSigningInformation[kSecCodeInfoTeamIdentifier as String] as? String
        else {
            throw Error
                .otherError(
                    "Failed to copy code signing information for unknown reason while trying to extract team ID",
                )
        }
