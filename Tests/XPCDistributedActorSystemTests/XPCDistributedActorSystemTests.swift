...
struct XPCDistributedActorSystemTests {
    ...
    func basicXPCTest() async throws {
        ...
        try await confirmation("creates remote actor just once") { confirmCreatesActor in
            let xpcHostDAS = XPCDistributedActorSystem(
                mode: .receivingConnections(listener: listenerXPC),
            ...
            let clientDAS = XPCDistributedActorSystem(
                mode: .connectingToEndpoint(endpoint: listenerXPC.endpoint),
                codeSigningRequirement: nil
            )
